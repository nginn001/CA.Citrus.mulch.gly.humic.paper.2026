...
library(grid)
###################################################
##### Greenhouse experiment final measurments #####
###################################################
#### Load data ####
plant<-read_excel("Sup.Table.S4 Greenhouse Plant Data.xlsx",col_names = TRUE)
...
m_sht <- lmer(sht_mass_g ~ mulch*Active.or.killed*humic+ (1|Block),data = plant.3) # reduced model
